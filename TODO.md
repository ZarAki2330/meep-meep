...
- **Réalisé** (90 tâches) : **~228 h** — l'essentiel : le catalogue de 390 jeux avec règles en français (~106 h à lui seul), les 5 modes de score, extensions/éditions, historique et statistiques, thème + accessibilité, refonte de l'écran d'ajout, avatars de personnages, étude règles PDF, tests.
- **Restant** (9 tâches) : **~26 h** (hors tâche récurrente « ajouter des jeux »).
- **Total projet** : **~254 h**.
...
- [x] **Protéger la propriété de l'application** — guide de protection rédigé (`docs/protection-propriete.md`) + fichier `LICENSE` propriétaire (« tous droits réservés », FR/EN) créé. À retenir : l'app est **déjà** ta propriété (droit d'auteur automatique sur code/design/logo/règles). Actions : (1) `LICENSE` + mention « © 2026 [nom] — Tous droits réservés » dans README et app _(gratuit, fait/à coller)_ ; (2) passer le dépôt GitHub en **privé** _(gratuit — attention à réhéberger `catalogue.json` avant)_ ; (3) **e-Soleau INPI 15 €** pour la preuve d'antériorité ; (4) **dépôt de marque INPI ~190 €** pour le nom « Meep Meep » _(seulement si commercialisation, vérifier dispo + risque « Bip Bip »)_ ; (5) ⚠️ avant de monétiser : régler les **images d'éditeurs** (jaquettes/pions sous copyright tiers) et préparer mentions légales + politique de confidentialité. Non juriste — avis pro recommandé pour le commercial.
...
- [x] **Afficher les règles officielles en PDF dans l'app (étude)** — étude de faisabilité rédigée (`docs/etude-regles-pdf.md`). Conclusion : le point bloquant est juridique — on ne peut pas héberger/embarquer le PDF officiel d'un éditeur (copyright), mais on peut **lier vers le fichier officiel de l'éditeur** ou laisser l'utilisateur **importer son propre PDF**. Recommandation par étapes : (1) champ `reglesUrl` + bouton « Règles officielles » ouvrant le lien (`expo-linking`/`expo-web-browser`, déjà faisable sans build) ; (2) import perso via `document-picker`/`file-system`/`sharing` (déjà installés) ; (3) visionneuse intégrée `react-native-pdf` seulement en v2 (build EAS, à valider sur Expo 54 + New Architecture). Le résumé de règles maison reste la colonne vertébrale. ✅ **Étape 1 implémentée** : champ `reglesUrl` ajouté au type `Jeu` et au formulaire d'ajout (section Présentation) ; sur la fiche, un bouton « Règles officielles (éditeur) » ouvre le lien via `Linking.openURL` (natif, zéro dépendance ajoutée, marche dans Expo Go) — s'affiche uniquement si le lien est renseigné. Le lien voyage aussi dans le partage de jeu. Restent optionnelles les étapes 2 (import PDF perso) et 3 (visionneuse intégrée).
- [x] **Compléter en masse les liens de règles officielles (Gigamic)** — recensement terminé (juil. 2026) via un workflow multi‑agents relancé **par lots de 60** avec écriture après chaque lot (méthode résiliente adoptée après qu'un premier gros workflow ait été perdu lors d'une réinitialisation du conteneur). Sur les 237 jeux Gigamic de base, **166 ont désormais un lien de règles officiel** (`reglesUrl` = `gigamic.com/index.php?controller=attachment&id_attachment=N`, le PDF hébergé par Gigamic) ; 71 sont restés sans lien (pas de page produit fiable ou pas de PDF, aucune URL devinée). Au total **183 des 390 jeux** ont un lien de règles (166 Gigamic + 11 grands titres + 6 distribués). Les liens ouvrent via le bouton « Règles officielles (éditeur) » sur la fiche.
- [x] **Améliorer les interfaces de l'app — personnages** — chaque personnage a désormais un avatar : sa photo/logo s'il en a un (nouveau 5e champ « image » dans le formulaire d'ajout), sinon une pastille colorée à son initiale (couleur déterministe tirée du nom, `lib/couleur-jeu.ts` → `couleurRole`). Nouveau composant `components/avatar-role.tsx`, affiché sur la fiche du jeu (`app/jeu/[id].tsx`) et dans les sélecteurs de rôle (`app/objectif`, `app/coop`). _Écran d'accueil (juil. 2026)_ : état vide repensé — une ludothèque vide (premier lancement, plus de jeux pré-installés) affiche désormais une invitation « Ta ludothèque est vide » avec une icône dé et un bouton « Ajouter un jeu », au lieu du message trompeur « aucun résultat ». Cas « aucun résultat de filtre » distingué, avec un bouton « Réinitialiser » (`app/(tabs)/index.tsx`). _Fiche d'un jeu (juil. 2026)_ : ligne joueurs/durée/âge enrichie d'icônes (personnes, horloge, gâteau) et le label passe à « équipes » pour les jeux en équipes ; bouton « Lancer / Reprendre la partie » doté d'une vraie icône « play » au lieu du caractère ▶ (`app/jeu/[id].tsx`, +2 icônes dans `components/ui/icon-symbol.tsx`). _Historique / statistiques (juil. 2026)_ : bandeau de synthèse passé à 3 tuiles (parties, jeux joués, meilleur joueur) avec icônes thématiques, et état vide illustré d'une icône (`app/(tabs)/explore.tsx`). _Compteur de points (juil. 2026)_ : classement en direct — chaque carte joueur affiche un badge de rang (1, 2, 3…) calculé selon le sens du score (max ou min pénalités), les ex æquo partageant le rang ; le 1er est mis en vert comme la surbrillance de tête (`app/partie/[jeuId].tsx`). Le badge n'apparaît que lorsque les scores diffèrent. Reste possible : étendre le même classement aux écrans Manches et Feuille de score.
- [x] **Revoir le design de l'application et le logo** — nouveau logo « meeple » : un meeple penché avec lignes de vitesse (piste D1). Dans l'en-tête d'accueil (`app/(tabs)/index.tsx`), deux calques superposés — meeple teinté par la couleur d'accent du thème (s'adapte au clair/sombre et à l'accent choisi) + lignes ambre fixes — suivis de « Meep Meep » en Fredoka (couleur d'accent). Icône de l'app, icône adaptative Android (fond violet dans `app.json`) et écran de démarrage refaits dans le même style (fond violet, meeple crème, lignes ambre) — visibles uniquement en build EAS, pas dans Expo Go. Logo source livré aussi en SVG (`meep-meep-logo.svg`). Reste possible : harmoniser plus largement la palette/typo sur tous les écrans.
