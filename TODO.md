...
- [ ] **Compléter en masse les liens de règles officielles (Gigamic)** — récupérer le `id_attachment` du PDF de règles de chaque jeu Gigamic sur gigamic.com et remplir `reglesUrl` dans `catalogue.json`. ~237 jeux Gigamic de base concernés (les 6 distribués et 11 grands titres non‑Gigamic sont déjà faits). ⚠️ À relancer **par lots** (~60 jeux) en écrivant les liens après chaque lot : un premier workflow massif (237 agents d'un coup) a été perdu quand le conteneur cloud a été réinitialisé en cours de route. Chaque lien devra être vérifié (pas d'URL devinée). Le script generateur est à recréer (perdu lors de la réinitialisation).
